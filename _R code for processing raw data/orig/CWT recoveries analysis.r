...
################### Go get the tag code file
 # this data comes from the "CWT release group identify.r"
	tag.dat	<- read.csv("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Releases/Tag codes fall chinook.csv")
		...
		save(releases,file=paste("/Users/ole.shelton/GitHub/Orca_Salmon/_Simulation and Analysis/Releases and Recoveries/Releases.RData",sep=""))	    
...
###########################################################################################################################################		
	...
	dat.loc.key	<- read.csv("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Recoveries/recovery codes-wietkamp+shelton 12-2015.csv",header=T)

################## First combine all of the recovery data files
setwd("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Recoveries/Fall Chinook/")
files	<- dir()
...
for(i in 1:length(files)){
	...
	if(i==11){
	  temp.tag <- temp$tag_code
	  temp.tag[nchar(temp.tag)==9] <- paste("0",temp.tag[nchar(temp.tag)==9],sep="")
	  temp$tag_code <- temp.tag
	}

	temp.1 <- temp[is.na(match(temp$tag_code,code.all))==F,]
# 	    print(files[i])
# 	    print(unique(temp$tag_code))
...
# 		C	<-	aggregate(A$recovery_location_code,by=list(A$recovery_location_code,A$recovery_location_name,A$fishery),length)
...
##### Combine Areas
#####################################################################################
	source(paste(base.dir,"/Orca_Salmon_Code/_R code for processing raw data/Combine recover areas.r",sep=""))
#####################################################################################
