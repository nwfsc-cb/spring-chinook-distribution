...
chosen.release	<-	read.csv("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Releases/river releases fall chinook.csv")
...
for(i in 1:nrow(chosen.release)){
	...
	trim	<-	temp[,c("ID","river","ocean.region","run","hatchery_location_code","tag_code_or_release_id",
...
focal.releases <- focal.releases[focal.releases$tag_code != "074627" &
									focal.releases$tag_code != "051536" &
...
write.csv(focal.releases,file = "Tag codes fall chinook.csv",row.names=F)
