...
names(pred) <- paste("X",loc.init,sep=".")
pred
### Make some plots

par(mfrow=c(3,1),mar=c(3,3,0.5,0.5))
these <- sort(sample(1:N.LOC.INIT,3))
...
param_m <- mvrnorm(10000,MU_m,Sigma_m)
    ...
    temp <-  param_m[,1] + param_m[,2] %*% t( month) 
    temp <- exp(-exp(temp))
    surv.all <- data.frame(surv.1=apply(temp[,1:12],1,prod),
                          surv.2=apply(temp[,13:24],1,prod), 
                          surv.3=apply(temp[,25:36],1,prod),
                          surv.4=apply(temp[,37:48],1,prod))
