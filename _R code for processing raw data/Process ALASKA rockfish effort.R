...
setwd(data.dir)
rockfish_observed<-readRDS("Unique_trips_Observed.RDS") %>%
  rename_all(tolower) %>% 
...
year.month<- expand.grid(temp.year, temp.month, pscnq_processing_sector=category,region= temp.region)  #fully factorial match between all categories 
...
rockfish_observed %>%  
  filter(year>2012) %>%
  ungroup() %>%
  dplyr::select(year,month,region,pscnq_processing_sector,observed_boat_days) %>%   
...
temp<- expand.grid(year=temp.year, region=temp.region, pscnq_processing_sector=category) #fully factorial match between all categories 
...
# year.month<- expand.grid(temp.year, temp.month, region=temp.region) #fully factorial match between all categories 
# year.month$Var2<- str_pad(year.month$Var2, width= 2, pad = "0", side="left") #add a zero before month 1-9 so that I can keep months in order
# year.month <- unite(year.month, "year.month", c("Var1", "Var2"), sep = ".", remove= TRUE) #join month and year in its own dataframe 

# observed_rockfish_2010_2012 %>%  
#   ungroup() %>%
#   dplyr::select(year,month,region,observed_boat_days) %>%   
#   mutate(month = str_pad(month, "left", pad=0, width = 2)) %>%
#   unite("year.month",c("year","month"), sep = ".") %>%
#   # gather(4:5, key = "category", value = "boat_days") %>%
#   merge(year.month, all = TRUE) %>% # add in labels
...
rockfish_fish_ticket<-readRDS("fishtix_snippet.RDS") %>% 
...
rockfish_fish_ticket %>%  
  ungroup() %>%
  dplyr::select(year,month,region,total_boat_days) %>%   
  mutate(month = str_pad(month, "left", pad=0, width = 2)) %>%
  unite("year.month",c("year","month"), sep = ".") %>%
  merge(year.month, all = TRUE) %>% # add in labels
...
temp<- expand.grid(year=temp.year, region=temp.region)  #fully factorial match between all categories 
...
#load RMIS data 
setwd(data.cwt.dir)
rockfish <- read.csv("Rockfish CWT.csv") %>%
  ...
  dplyr::mutate(region = case_when(Area == 620 ~ "E.APEN",   
                                   Area == 630 ~ "NW.GOA",
                                   Long <= -147 & Long > -154 ~ "NW.GOA", #"X630",
                                   Long <= -154 & Long > -159  ~ "W.APEN", #"X620",
...
saveRDS(rockfish_observed, "data/AK_CWT_Rockfish/rockfish_2013_2017_summarized.RDS") 
