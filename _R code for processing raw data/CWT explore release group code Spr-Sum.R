...
B <- aggregate(dat.release$hatchery_location_code, by = list(hatch.loc=dat.release$hatchery_location_code,
                                                          stock.name = dat.release$stock_location_name,
                                                          state = dat.release$release_location_state,
                                                          run = dat.release$run),length)
write.csv(B,file="all chinook release hatchery names.csv")

C <- aggregate(dat.release$hatchery_location_code, by = list(stock.loc=dat.release$stock_location_code,
                                                             hatch.loc=dat.release$hatchery_location_code,
                                                             #stock.name = dat.release$stock_location_name,
                                                             state = dat.release$release_location_state,
                                                             run = dat.release$run),length)
C <- C %>% filter(run>=3)

write.csv(C,file="all chinook release hatchery names short.csv")
...
B <- aggregate(dat.release[,c('cwt_1st_mark_count','cwt_2nd_mark_count')],
		by = list(stock.loc	   			= dat.release$stock_location_code,
					stock.name 			= dat.release$stock_location_name,
					hatch.code      = dat.release$hatchery_location_code,
					release.loc.name 	= dat.release$release_location_name,
					state	   			= dat.release$release_location_state,
					brood.year 			= dat.release$brood_year, 
					run=dat.release$run, 
...
### Examine only Fall, Summer, late Fall runs (runs 2,3,7)

dat.fall      <- dat.rel.fin %>% filter(run==c(3))
dat.sum       <- dat.rel.fin %>% filter(run==c(2))
dat.lt.fall   <- dat.rel.fin %>% filter(run==c(7))

dat.all <- rbind(dat.fall,dat.sum)
dat.all <- rbind(dat.all,dat.lt.fall)

#### Isolate Columbia river stocks: WA,OR,ID releases
#dat.COL   <- dat.fall %>% filter(state != "BC") %>% filter(state!="CA")
IDs       <- unique(dat.all$stock.loc)
stock.IDs <- data.frame(cbind(IDs,dat.all[match(IDs,dat.all$stock.loc),]))

IDs       <- unique(dat.all$hatch.code)
hatch.IDs <- data.frame(cbind(IDs,dat.all[match(IDs,dat.all$hatch.code),]))


setwd("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Releases/Climate releases")
write.csv(stock.IDs,file="Unique releases sum-fall-ltfall.csv",row.names=F)
write.csv(hatch.IDs,file="Unique releases-hatch sum-fall-ltfall.csv",row.names=F)
