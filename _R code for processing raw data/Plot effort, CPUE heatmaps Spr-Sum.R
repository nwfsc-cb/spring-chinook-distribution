...
if(all.seas==TRUE){axis(1,las=2,at=seq(2,ncol(temp),by=4),labels=YEARS.RECOVER)}
...
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/Effort heatmap ",SEAS[i],".pdf",sep=""),height = 8.5, width=11)
...
# 
# pdf(paste(base.dir,"/Orca_Salmon/Output plots/"," Troll + Treaty observations heatmap.pdf",sep=""),height = 8.5, width=8.5)
#     plot.heatmap.nolog( temp.all=C_troll_true,id=73)
#     plot.heatmap.nolog( temp.all=C_troll_true,id=323)
#     plot.heatmap.nolog( temp.all=C_troll_true,id=51)
#     plot.heatmap.nolog( temp.all=C_troll_true,id=124)
#     plot.heatmap.nolog( temp.all=C_troll_true,id= 145)
#     plot.heatmap.nolog( temp.all=C_troll_true,id= 251)
#     plot.heatmap.nolog( temp.all=C_troll_true,id= 264)
# dev.off()
  
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/All Troll + Treaty observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
  for(i in 1:N.REL){
    plot.heatmap.nolog( temp.all=C_troll_true,id=i)
  }
dev.off()
# 
# 
# pdf(paste(base.dir,"/Orca_Salmon/Output plots/"," Rec observations heatmap.pdf",sep=""),height = 8.5, width=8.5)
#   plot.heatmap.nolog( temp.all=C_rec_true,id=73)
#   plot.heatmap.nolog( temp.all=C_rec_true,id=323)
#   plot.heatmap.nolog( temp.all=C_rec_true,id=51)
#   plot.heatmap.nolog( temp.all=C_rec_true,id=124)
#   plot.heatmap.nolog( temp.all=C_rec_true,id= 145)
#   plot.heatmap.nolog( temp.all=C_rec_true,id= 251)
#   plot.heatmap.nolog( temp.all=C_rec_true,id= 264)
# dev.off()

pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/All Rec observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
  plot.heatmap.nolog( temp.all=C_rec_true,id=i)
}
dev.off()

pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/All Trawl ASHOP observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
  plot.heatmap.nolog( temp.all=C_hake_ashop_true,id=i)
}
dev.off()

pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/All Trawl Shoreside observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
...
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/Sample Fraction heatmap.pdf",sep=""),height = 8.5, width=11)
...
plot.CPUE.heatmap <- function(temp.all,effort.all,id,MONTH.STRUCTURE){
  ...
  if(REL$brood_year[id]+2 >= 1979){
    if(MONTH.STRUCTURE=="FOUR"){e.start <- grep(paste(REL$brood_year[i]+2,".month.spring",sep=""),rownames(effort.all))}
  ...
  lab.new <- LOCATIONS.plot[order(LOCATIONS$location.number),]
  x.lab	=	lab.new$location.name
  col.br<- colorRampPalette(c("blue", "cyan", "yellow", "red"))
  par(mfrow=c(1,1),oma=c( 0,0,0,4) )
...
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
  for(i in 1:N.REL){
...
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Rec observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
...
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Rec Can observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
...
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Rec Can iREC observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
...
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Treaty observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
  plot.CPUE.heatmap( temp.all=C_treaty_true,effort.all= K_treaty_flat,id=i,MONTH.STRUCTURE=MONTH.STRUCTURE)
}
dev.off()
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Trawl ASHOP observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
  plot.CPUE.heatmap( temp.all=C_hake_ashop_true,effort.all= K_hake_ashop_flat,id=i,MONTH.STRUCTURE=MONTH.STRUCTURE)
}
dev.off()
#################################
pdf(paste(base.dir,"/Salmon-Climate/Heatmaps/CPUE Trawl Shoreside observations heatmap ",GROUP,".pdf",sep=""),height = 8.5, width=8.5)
for(i in 1:N.REL){
...
quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Coleman 1980.pdf"),height=4,width=5,dpi=600,type="pdf")
      plot.CPUE.heatmap2( temp.all=C_troll_true,effort.all= K_troll_flat,id=235)
dev.off()

quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Coleman 1999.pdf"),height=4,width=5,dpi=600,type="pdf")
  plot.CPUE.heatmap2( temp.all=C_troll_true,effort.all= K_troll_flat,id=254)
dev.off()

# read in files combine into side by side figures
p1 <- ggdraw() + draw_image(magick::image_read_pdf(paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Coleman 1980.pdf"), 
                                                   density = 600))
p2 <- ggdraw() + draw_image(magick::image_read_pdf(paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Coleman 1999.pdf"), 
                                                   density = 600))

quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Coleman Combined.pdf"),height=4,width=10,dpi=600,type="pdf")
  print(plot_grid(p1,p2))
dev.off()

# LCOL "Big Creek_small" 1980 and 1999 are ID = 34,48
quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Big Creek 1980.pdf"),height=4,width=5,dpi=600,type="pdf")
  plot.CPUE.heatmap2( temp.all=C_troll_true,effort.all= K_troll_flat,id=34)
dev.off()

quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Big Creek 1999.pdf"),height=4,width=5,dpi=600,type="pdf")
  plot.CPUE.heatmap2( temp.all=C_troll_true,effort.all= K_troll_flat,id=48)
dev.off()

# read in files combine into side by side figures
p1 <- ggdraw() + draw_image(magick::image_read_pdf(paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Big Creek 1980.pdf"), 
                                                   density = 600))
p2 <- ggdraw() + draw_image(magick::image_read_pdf(paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Big Creek 1999.pdf"), 
                                                   density = 600))

quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Big Creek Combined.pdf"),height=4,width=10,dpi=600,type="pdf")
  print(plot_grid(p1,p2))
dev.off()

# URB "Priest" 1980 and 1999 are ID = 34,48
quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Priest 1980.pdf"),height=4,width=5,dpi=600,type="pdf")
  plot.CPUE.heatmap2( temp.all=C_troll_true,effort.all= K_troll_flat,id=901)
dev.off()

quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Priest 1999.pdf"),height=4,width=5,dpi=600,type="pdf")
  plot.CPUE.heatmap2( temp.all=C_troll_true,effort.all= K_troll_flat,id=920)
dev.off()

# read in files combine into side by side figures
p1 <- ggdraw() + draw_image(magick::image_read_pdf(paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Priest 1980.pdf"), 
                                                   density = 600))
p2 <- ggdraw() + draw_image(magick::image_read_pdf(paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Priest 1999.pdf"), 
                                                   density = 600))

quartz(file=paste0(base.dir,"/Salmon-Climate/Heatmaps/CPUE Troll observations heatmap PUB Priest Combined.pdf"),height=4,width=10,dpi=600,type="pdf")
print(plot_grid(p1,p2))
dev.off()
