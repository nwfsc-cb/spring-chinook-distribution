...
pdf(paste(base.dir,"/Salmon-Climate/Output plots/"," Juv mortality plots ",NAME,".pdf",sep=""),height = 6, width=8.5)
...
for(i in 1:nrow(spawn_loc)){
  ...
  plot(x= REL$brood_year[REL$ocean.region==spawn_loc$ocean.region[i]],y=exp(-rel_year_all[REL$ocean.region==spawn_loc$ocean.region[i]]),  
          ylab="First year survivorship (S)",xlab="Brood year",ylim=c(0,exp(-min(rel_year_all))),xlim=c(min(REL$brood_year),max(REL$brood_year)))

  plot(exp(-rel_year_all[REL$ocean.region==spawn_loc$ocean.region[i]])~REL$n.month[REL$ocean.region==spawn_loc$ocean.region[i]],
       ylab="Median survivorship",xlab="Juvenile period (months)", ylim=c(0,exp(-min(rel_year_all))),xlim=c(2,13))
  }
...
P.juv <- ggplot() +
      ...
      scale_x_continuous(limits=c(0,8),breaks=0:8,expand=c(0,0.1)) +
...
quartz(file=paste(base.dir,"/Salmon-Climate/Output plots/Juv Mort Histogram ",NAME,".jpeg",sep=""),type="jpeg",dpi=600,width=4,height=3)
  print(P.juv)
dev.off()
...
#### START PLOT
quartz(file=paste(base.dir,"/Salmon-Climate/Output plots/Juv Mort Pub plot ",NAME,".jpeg",sep=""),type="jpeg",dpi=600,width=4,height=3)
...
#### START PLOT
quartz(file=paste(base.dir,"/Salmon-Climate/Output plots/Juv Mort Pub plot ALL ",NAME,".jpeg",sep=""),type="jpeg",dpi=600,width=4,height=3)
...
#V2 Single Panel 
quartz(file=paste(base.dir,"/Salmon-Climate/Output plots/Juv Mort weighted pub plot",NAME,".jpeg",sep=""),type="jpeg",dpi=600,width=4,height=4)
