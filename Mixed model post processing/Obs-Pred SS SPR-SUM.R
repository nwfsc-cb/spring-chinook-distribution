...
pollock_GOA_mat <- vuln_troll_array * 0
rockfish_AK_mat <- vuln_troll_array * 0
...
if(vuln_fixed>2){ # THis is for the logistic formulation
  for(i in 1:N_years_release){ ## Make an array of the same dimensions as the state vector
    ...
    if(length(beta_vuln_hake)==2){
      hake_ashop_mat[i,,]     = 1 / (1 + exp(-(vuln_fixed + (beta_vuln_hake[1]  * vuln_age_hake + beta_vuln_hake[2]  * vuln_age_hake^2))))
      hake_shoreside_mat[i,,] = 1 / (1 + exp(-(vuln_fixed + (beta_vuln_hake[1]  * vuln_age_hake + beta_vuln_hake[2]  * vuln_age_hake^2))))
    }
    if(length(beta_vuln_pollock)==1){
      pollock_GOA_mat[i,,]     = 1 / (1 + exp(-(vuln_fixed + (beta_vuln_pollock  * vuln_age_pollock))))
      rockfish_AK_mat[i,,] = 1 / (1 + exp(-(vuln_fixed + (beta_vuln_pollock  * vuln_age_pollock))))
    }
    if(length(beta_vuln_pollock)==2){
      pollock_GOA_mat[i,,]     = 1 / (1 + exp(-(vuln_fixed + (beta_vuln_pollock[1]  * vuln_age_pollock + beta_vuln_pollock[2]  * vuln_age_pollock^2))))
      rockfish_AK_mat[i,,] = 1 / (1 + exp(-(vuln_fixed + (beta_vuln_pollock[1]  * vuln_age_pollock + beta_vuln_pollock[2]  * vuln_age_pollock^2))))
    }
    }
  }
}
...
F_pollock_GOA_fin <- F_troll_fin
F_rockfish_AK_fin <- F_troll_fin
...
for(i in 1:N_years_release){ # Make an array of the same dimensions as the state vector
  ...
  F_pollock_GOA_fin[,,i]    <- F_pollock_GOA_array[START:STOP,] * pollock_GOA_mat[i,,]  ;
  F_rockfish_AK_fin[,,i]    <- F_rockfish_AK_array[START:STOP,] * rockfish_AK_mat[i,,]  ;

  F_tot_fin[,,i]        <- F_troll_fin[,,i] + F_rec_fin[,,i] + F_treaty_fin[,,i] + 
                                  F_hake_ashop_fin[,,i] + F_hake_shoreside_fin[,,i] +
...
for(i in 1:N_obs_all){
  ...
  if( gear_bin_idx[i]  == 6 ){ F_focal <- F_pollock_GOA_fin[mod_time_idx[i],loc_idx[i],start_year[rel_idx[i]]] ; }   
  if( gear_bin_idx[i]  == 7 ){ F_focal <- F_rockfish_AK_fin[mod_time_idx[i],loc_idx[i],start_year[rel_idx[i]]] ; }   
  ...
  if(spawn_time_array[rel_idx[i] ,mod_time_idx[i]] == 0){
    lambda_temp <- #logit_offset_int[i]  + logit_offset_slope *
        Baranov(  cum_M2_temp[mod_time_idx[i]],
                  ...
                  F_tot,
                  log_N_temp_2,
                  log(origin_loc_mean[origin_bin_idx[i],season_bin_idx[i],loc_idx[i]] ) );
  }
  if(spawn_time_array[rel_idx[i] ,mod_time_idx[i]] > 0){
    ...
    log_N_temp_1 <- log_N_temp_2 - 
                  ...
                  log(1- exp(log(prob_age_year[origin_bin_idx[i],juv_bin_idx[i], spawn_time_array[rel_idx[i],mod_time_idx[i]]]) - 
                               river_entry[rel_idx[i],loc_idx[i]] * (spawn_smooth)^(-1))) ;
...
pdf(paste(base.dir,"/spring-chinook-distribution/Output plots/", "Obs-Pred Binomial " ,NAME,".pdf",sep=""),height = 5, width=7)
  ...
  title("Rec")
  pred.obs.plot(data=dat.all[dat.all$effort.type == "hake_ashop",],prob=dat.all$prob[dat.all$effort.type == "hake_ashop"],smoother.int=INT)
  title("Hake ASHOP")
  pred.obs.plot(data=dat.all[dat.all$effort.type == "hake_shoreside",],prob=dat.all$prob[dat.all$effort.type == "hake_shoreside"],smoother.int=INT)
  title("Hake Shoreside")
  pred.obs.plot(data=dat.all[dat.all$effort.type == "pollock_GOA",],prob=dat.all$prob[dat.all$effort.type == "pollock_GOA"],smoother.int=INT)
  title("Pollock GOA")
  pred.obs.plot(data=dat.all[dat.all$effort.type == "rockfish_AK",],prob=dat.all$prob[dat.all$effort.type == "rockfish_AK"],smoother.int=INT)
  title("Rockfish AK")
  ...
  for(i in 1:(length(BY)-1)){
    ...
    d.temp <- dat.all %>% filter(effort.type =="hake_ashop",brood_year >= BY[i],brood_year < BY[i+1])
    pred.obs.plot(data=d.temp,prob=d.temp$prob,smoother.int=INT)
    title(paste("Hake ASHOP",BY[i],"to",BY[i+1]-1))
    
    d.temp <- dat.all %>% filter(effort.type =="hake_shoreside",brood_year >= BY[i],brood_year < BY[i+1])
    pred.obs.plot(data=d.temp,prob=d.temp$prob,smoother.int=INT)
    title(paste("Hake Shore",BY[i],"to",BY[i+1]-1))
    
    d.temp <- dat.all %>% filter(effort.type =="pollock_GOA",brood_year >= BY[i],brood_year < BY[i+1])
    pred.obs.plot(data=d.temp,prob=d.temp$prob,smoother.int=INT)
    title(paste("Pollock GOA",BY[i],"to",BY[i+1]-1))
    
    d.temp <- dat.all %>% filter(effort.type =="rockfish_AK",brood_year >= BY[i],brood_year < BY[i+1])
    pred.obs.plot(data=d.temp,prob=d.temp$prob,smoother.int=INT)
    title(paste("Rockfish AK",BY[i],"to",BY[i+1]-1))
  ...
  for(i in 1:4){
    ...
    mtext(paste("1=spr, 2=sum, 3=fall, 4=wint"),padj=-0.5)
  }
dev.off()
...
pdf(paste(base.dir,"/spring-chinook-distribution/Output plots/", "Obs-Pred Positive " ,NAME,".pdf",sep=""),height = 5, width=9)
...
title("Rec")
pred.obs.pos.plot(data=dat.pos[dat.pos$effort.type == "hake_ashop",],pred.pos=(dat.pos$E_pred[dat.pos$effort.type == "hake_ashop"]),pch=21)
title("ASHOP (hake)")
pred.obs.pos.plot(data=dat.pos[dat.pos$effort.type == "hake_shoreside",],pred.pos=(dat.pos$E_pred[dat.pos$effort.type == "hake_shoreside"]),pch=21)
title("Shoreside (hake)")
pred.obs.pos.plot(data=dat.pos[dat.pos$effort.type == "pollock_GOA",],pred.pos=(dat.pos$E_pred[dat.pos$effort.type == "pollock_GOA"]),pch=21)
title("Pollock GOA")
pred.obs.pos.plot(data=dat.pos[dat.pos$effort.type == "rockfish_AK",],pred.pos=(dat.pos$E_pred[dat.pos$effort.type == "rockfish_AK"]),pch=21)
title("Rockfish AK")
...
for(i in 1:N_season){
  ...
  title(paste("Season =",i))
  mtext(paste("1=spr, 2=sum, 3=fall, 4=winter"),padj=-0.5)
}
...
quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-all " ,NAME,".jpeg",sep=""),height=5,width=5,dpi=DPI,type="jpeg")
  print(all.bin)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Pos-all " ,NAME,".jpeg",sep=""),height=5,width=5,dpi=DPI,type="jpeg")
  print(all.pos)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-facet-origin " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.bin.origin.facet)
dev.off()
 
quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-facet-time " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.bin.time.facet)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-facet-time-origin " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.bin.time.facet.origin)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-facet-time-region " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.bin.time.facet.reg)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Pos-facet-origin " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.pos.origin.facet)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Pos-facet-time " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.pos.time.facet)
dev.off()

quartz(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Pos-facet-time-region " ,NAME,".jpeg",sep=""),height=7,width=7,dpi=DPI,type="jpeg")
  print(all.pos.time.facet.reg)
dev.off()


pdf(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-facet-time-region-by-origin " ,NAME,".pdf",sep=""),
    onefile=T)
  print(all.bin.time.facet.reg.by.origin)
dev.off()

pdf(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Pos-facet-time-region-by-origin " ,NAME,".pdf",sep=""),
    onefile=T)
  print(all.pos.time.facet.reg.by.origin)
dev.off()

pdf(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Bin-facet-time-region-by-origin.BW " ,NAME,".pdf",sep=""),
    onefile=T)
print(all.bin.time.facet.reg.by.origin.bw)
dev.off()

pdf(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Obs-Pred Pos-facet-time-region-by-origin.BW " ,NAME,".pdf",sep=""),
    onefile=T)
