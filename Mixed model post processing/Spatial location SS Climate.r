...
#if(SEASON == TRUE){  
    pdf(paste(base.dir,"/Salmon-Climate/Output plots/","Distribution heatmap ",NAME,".pdf",sep=""),height = 6, width=8.5)
...
###################################################################################################
  ...
  quartz(file=paste(base.dir,"/Salmon-Climate/Output plots/Spatial Average Distribution plot ",NAME,".jpeg",sep=""),
           type="jpeg",dpi=600,width=4.5,height=6)
  
par(mfrow=c(3,1))
TEXT <- c("a) Winter-Spring","b) Summer", "c) Fall")
...
for(i in 1:3){  
  ...
  if(i==3){title(xlab="Ocean region",line=2.25,cex.lab=1)}
  #title(main=title)
  #par(mfrow=c(1,1),oma=c( 0,0,0,0) )
  ...
  #COL <- brewer.pal(32,Yl)
    ...
    z.lim	<- c(0.0,0.40)
    y.lab <- spawn_loc$ocean.region
    x.lab	<-	LOCATIONS.plot$location.name
    #col.br <- colorRampPalette(c("blue", "cyan", "yellow", "red"))
    col.br <- colorRampPalette(c(grey(0.9),"orange","orangered", "red3"))
    #col.br <- colorRampPalette(COL)
