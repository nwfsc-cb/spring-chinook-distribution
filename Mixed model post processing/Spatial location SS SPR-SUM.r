...
#############################################################3  
  ...
  temp1[temp1 > z.lim[2]] <- z.lim[2]-1e-8
  #par(oma=c( 0,0,0,4) )
  image(z=t(temp1),y=1:nrow(temp1),x=LOCATIONS$location.number,axes=F,ylab="",xlab="",
        col=col.br(32),zlim=z.lim)
  box(bty="o",lwd=2)
  ...
  image.plot(temp1,legend.only=T,smallplot=c(0.45,0.60, 0.25,0.9),
             col=col.br(32),
             zlim=z.lim,
             axis.args=list(at= ticks,labels=ticks.lab,
                            cex.axis=0.5,hadj=0.8,tcl=-0.3),add=T,legend.cex=0.7)
  ...
  #COL <- brewer.pal(32,Yl)
    ...
    # Color Bar
      ticks <- seq(min(z.lim),max(z.lim),length.out=6)
      ticks.lab <- ticks; ticks.lab[6] <- paste0(">",ticks.lab[6])
      image.plot(temp1,legend.only=T,smallplot=c(0.45,0.60, 0.25,0.9),col=col.br(32),
...
#   
#   
#   pdf(file=paste(base.dir,"/spring-chinook-distribution/Output plots/Distribution by origin, centered ",NAME,".pdf",sep=""),
