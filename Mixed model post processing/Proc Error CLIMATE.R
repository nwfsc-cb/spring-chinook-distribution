...
pdf(paste(base.dir,"/Salmon-Climate/Output plots/Process error ",NAME,".pdf",sep=""),height = 5, width=10,onefile=T)
...
pdf(paste(base.dir,"/Salmon-Climate/Output plots/Process error 2 ",NAME,".pdf",sep=""),height = 5, width=10,onefile=T)
  #### MAKE HEATMAPS FOR ALL RELEASES
...
epsilon.dat2$ocean.region <- factor(epsilon.dat2$ocean.region,
                                   levels = c("SWVI  ", "SGEO","PUSO","LCOL","MCOL","UCOL",
                                              "SNAK","NCA","SFB"))
LIM=c(-1.1,1.1)
...
temp <- epsilon.dat2[epsilon.dat2$ocean.region =="LCOL" |
                      epsilon.dat2$ocean.region =="MCOL" |
                      epsilon.dat2$ocean.region =="UCOL" |
                      epsilon.dat2$ocean.region =="SNAK",]
temp <- summarise(group_by(temp,ocean.region,model.time),MEAN=mean(value),SD = sd(value))
temp <- temp[temp$model.time<19,]
colnames(temp)[grep("ocean",colnames(temp))] <- "Region"
...
temp <- epsilon.dat2[ epsilon.dat2$ocean.region =="NCA" |
...
quartz(file=paste(base.dir,"/Salmon-Climate/Output plots/Process error TIME SERIES ",NAME,".pdf",sep=""),height = 8, width=6,dpi=300,type="pdf")
