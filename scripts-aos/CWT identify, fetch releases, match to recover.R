...
GROUP <- "CA+COL"  ### Options "CA", "CA+" "COL" "CA+COL" "CA+COL_AWG" "CA+COL+PUSO"  # THIS IS WHAT YOU READ IN WITH
GROUP.2 <- "CA+COL"  # THIS IS WHAT GETS WRITTEN TO FILE.  Unless == "CA+COL_AWG" this should be the same as GROUP
                          #A CLUGE SO CAN READ IN THE SAME DATA BUT END UP WITH A SLIGHTLY DIFFERENT DATA FRAME
loc_18 <- "_18loc" ##_18loc" ## options for this are a null string ("", should be default) or "_18loc" for using 2 areas for PUSO.
...
NOM <- paste("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Releases/Climate/river releases fall chinook ",GROUP," CLIMATE.csv",sep="")
...
write.csv(focal.releases,
          file = paste("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Releases/Climate/Tag codes fall chinook ",GROUP," CLIMATE",loc_18,".csv",sep=""),row.names=F)
###############################################################################################
###############################################################################################
###############################################################################################
###############################################################################################
...
tag.dat	<- read.csv(paste("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Releases/Climate/Tag codes fall chinook ",GROUP," CLIMATE",loc_18,".csv",sep=""))
...
dat.loc.key	<- read.csv("/Users/ole.shelton/GitHub/Orca_Salmon_DATA/Recoveries/recovery codes-wietkamp+shelton 02-2018 two PUSO.csv",header=T)
...
# A <- NULL
# B <- NULL
# if(GROUP=="CA+COL" |GROUP.2=="CA+COL_AWG" ){
#   for(i in 1:length(files)){
#     temp	<-	read.csv(files[i])
#     
#     temp$tag_code <- as.character(temp$tag_code)
...
#     if(i==1){
#         tag.code.all <- unique(temp$tag_code)
#     }else{
#         temp <- temp %>% filter(!tag_code %in% tag.code.all )
#         tag.code.all <- c(tag.code.all,unique(temp$tag_code))
#     }
#     
#     temp.1 <- temp[is.na(match(temp$tag_code,code.all))==F,]
#     all.code.recoveries	<-	rbind(temp.1,all.code.recoveries)
#     
#     # A check to make sure no tag codes are duplicated by accident.
#     A <- c(A,unique(temp$tag_code))
#     B <- c(B,rep(i,length(unique(temp$tag_code))))
#   }
# }  
##########
...
##### Combine Areas
#####################################################################################
source(paste(base.dir,"/Salmon-Climate/_R code for processing raw data/Combine recover areas CLIMATE",loc_18,".r",sep=""))
#####################################################################################
...
save(ocean.recover,file=paste(base.dir,"/Salmon-Climate/Processed Data/Ocean Recoveries ",GROUP,loc_18,".RData",sep=""))	    
...
save(fresh.recover,file=paste(base.dir,"/Salmon-Climate/Processed Data/Fresh Recoveries ",GROUP,loc_18,".RData",sep=""))	    
####################################################
#############################################
