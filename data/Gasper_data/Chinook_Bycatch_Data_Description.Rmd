...
Okay, since the **rate_precendence** is what dictates the **rate** and thus, the estimated number of Chinook, let's look at this field. 
...
```{r}
data %>% 
  filter(trip_target_code=="K" & nmfsarea%in%c(610,620,630)) %>% 
  group_by(year) %>% 
  summarise(est_count=sum(pscnq_estimate,na.rm=TRUE),
            spp_count=sum(species_count,na.rm=TRUE)) %>% 
  data.frame
