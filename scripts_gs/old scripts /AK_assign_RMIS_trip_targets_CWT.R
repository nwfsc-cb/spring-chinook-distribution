...
observer<- observer.load %>% #this has cruise number which, in combo with other columns can be used to match. Cruise # is specific to an observer and their deployment so it can not be the only thing used to match. 
  ...
  filter(case_when(year > 2012 ~ !trip_target_code == "K",
                   TRUE ~ trip_target_code == trip_target_code)) %>% # RMIS starts coding rockfish specific recoveries in 2013, remove them from this data set so we dont get duplicates. 
...
bycatch.load <-  readRDS("data/AK_CWT_Trawl/Observer_data_base/bycatch_data_update.RDS") 
#deal with these seperately because info before 1991 isnot in observer database 

bycatch<- bycatch.load %>% #doesn't have specific trip info, just weekly summary, date is the end of week date. 
  ...
  rename(vessel_name=ves_akr_name, processing_sector=pscnq_processing_sector, year=year.1) %>% 
  filter(year > 1995,
         case_when(year > 2012 ~ !trip_target_code == "K",
                   TRUE ~ trip_target_code == trip_target_code)) %>% #our recovery data for rockfish are seperate, so dont need them here
  mutate(trip_target_code= case_when(trip_target_code %in% c("P","B") ~ "pollock",
  ...
  group_by(year,week_end_date, vessel_id, reporting_area_code,processing_sector,ves_akr_cg_num, ves_akr_adfg, #catcher_vessel_id, 
           #processor_permit_id,
