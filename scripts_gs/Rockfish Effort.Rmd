...
\n 2007-2012: Shoreside-- Have total effort here, don't know the fraction observed, use management code to ID trips.
...
\n 2010-2012: During the rockfish pilot program. Jordan suggests identifying trips based on the mangement code RPP, these entries start in 2010, but 2010-2012 is CP only, & because they dont use elandings fish ticket stuff I do not have info on unobserved effort for these, are they fully observed??
...
#2010 - 2012
observed_rockfish_2010_2012 <- rockfish_observed %>%
  filter(year > 2009 & year<2013)

#2013-2017
full_census_rockfish<-rockfish_observed %>%
  filter(year > 2012) 
...
### Observed CP 2010-2012
\n but we have no info about total effort in 2010-2012, were CP's fully observed? Can we get the total effort for these years? 
```{r}
  #2010 - 2012
...
### Total Effort Shoreside 2007-2012
\n Dont have information on observed effort during this time.
...
rockfish_fish_ticket %>%  
  ungroup() %>%
  select(year,month,region,total_effort) %>%   
  mutate(month = str_pad(month, "left", pad=0, width = 2)) %>%
  unite("year.month",c("year","month"), sep = ".") %>%
  merge(year.month, all = TRUE) %>% # add in labels
  mutate(total_effort= replace_na(total_effort,0)) %>%
    ggplot() +
      geom_bar(aes(x=year.month,y=total_effort), position="dodge", stat="identity",alpha = 0.8) +
      facet_wrap(~region, scales = "free") +
...
rockfish_fish_ticket %>%  
  ...
  mutate(total_effort= replace_na(total_effort,0)) %>%
   ggplot() +
    geom_bar(aes(x=year,y=total_effort),stat="identity",alpha = 0.8) +
    facet_wrap(~region, scales = "free") +
...
saveRDS(full_census_rockfish, "data/AK_CWT_Rockfish/rockfish_2013_2017_summarized.RDS")
```
